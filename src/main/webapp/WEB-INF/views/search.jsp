...
<div class="container">
	<div class="row mb-2">
        <div class="col-12 text-center">
        	<p class="fw-bold h2">"${keyword}" 검색 결과</p>
        	<!--
        ...
        <div class="col-xl-3 col-lg-4 col-md-6 mb-3">
            <div class="card" >
                ...
                <div class="card-body">
                    <h5 class="card-title">
                    	<c:choose>
	                		<c:when test="${dto.bGroup==1}">[Web]${dto.bTitle}</c:when>
	                		<c:when test="${dto.bGroup==2}">[Game]${dto.bTitle}</c:when>
	                		<c:when test="${dto.bGroup==3}">[Music]${dto.bTitle}</c:when>
	                	</c:choose>
                    </h5><a href="select_one?bId=${dto.bId}" class="stretched-link select_one" data-bs-toggle="modal" data-bs-target="#exampleModal"></a>
		...
		$(".select_one").click(function(event){
			...
			$.ajax({
				...
				success : function(result) {
					if(result.project.bGroup==1){
						$("#spTitle").text("[Web]"+result.project.bTitle);
					} else if(result.project.bGroup==2) {
						$("#spTitle").text("[Game]"+result.project.bTitle);
					} else {
						$("#spTitle").text("[Music]"+result.project.bTitle);
					}
